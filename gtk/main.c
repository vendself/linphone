...
gchar *aupkg_file = NULL;				// 2012/03/07, 실행 후 호 생성시 참조할 SDP 내용을 저장한 파일명
gboolean g_enable_video=FALSE;			// 2012/03/14, 실행시 Enable Video 옵션
...
static GOptionEntry linphone_options[]={
	...
	{	// 2012/03/07
	    ...
	    .short_name = 'm',
	    .arg = G_OPTION_ARG_STRING,
	    .arg_data = (gpointer) & aupkg_file,
	    .description = N_("Specifiy a aupkg file")
	},
	{	// 2012/03/14
		.long_name="enable-video",
		.short_name= 'v',
		.arg=G_OPTION_ARG_NONE,
		.arg_data= (gpointer)&g_enable_video,
		.description=N_("if set automatically enable video call")
	},
	{0}
};
...
static void linphone_gtk_call_state_changed(LinphoneCore *lc, LinphoneCall *call, LinphoneCallState cs, const char *msg){
	char	szCommand[1024] = "";
	FILE	*fp;
	char	_szAuPkgFile[256] = "";
	char	*progdir;
	int 	nRet = 0;
	
	memset(szCommand, 0, sizeof(szCommand));
	memset(_szAuPkgFile, 0, sizeof(_szAuPkgFile));
		...
		case LinphoneCallConnected:
			...
			if (progpath != NULL)
			{
				...
				if (basename != NULL)
				{
					basename ++;
					*basename = '\0';
					snprintf(_szAuPkgFile, sizeof(_szAuPkgFile), "%s\\..\\%s", progdir, aupkg_file);
				}
				if (workingdir!=NULL)
				{
					snprintf(_szAuPkgFile, sizeof(_szAuPkgFile), "%s\\%s", workingdir, aupkg_file);
				}
				free(progdir);
			}
			
			ms_message("LinphoneCallConnected, aupkg_file(%s).....", _szAuPkgFile);
			fp = fopen(_szAuPkgFile, "rt");
			if(fp != NULL)
			{
				nRet = fread(szCommand, sizeof(char), sizeof(szCommand), fp);
				ms_message("FileRead(%s)=%d,[%s]", _szAuPkgFile, nRet, szCommand);
				fclose(fp);
				linphone_core_send_aupkg(lc, szCommand);
			}
			else
			{
				ms_message("FileRead(%s) FAIL.....", _szAuPkgFile);
			}
...
// 2012/03/14, 비디오 사용 여부를 설정
void linphone_gtk_set_menu_items(bool_t video_enabled)
{
	bool_t selfview = FALSE;
	GtkWidget *main_window = NULL;
	GtkWidget *enable_video_item = NULL;
	GtkWidget *selfview_item = NULL;
	
	selfview = linphone_gtk_get_ui_config_int("videoselfview", VIDEOSELFVIEW_DEFAULT);
	
	main_window = linphone_gtk_get_main_window();
	if(main_window != NULL)
	{
		enable_video_item = linphone_gtk_get_widget(linphone_gtk_get_main_window(), "enable_video_item");
		if(enable_video_item != NULL)
		{
			gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(enable_video_item), video_enabled);
		}
		else
		{
			ms_error("%s, enable_video_item is NULL", __func__);
		}
		
		selfview_item = linphone_gtk_get_widget(linphone_gtk_get_main_window(), "selfview_item");
		if(selfview_item != NULL)
		{
			gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(selfview_item), selfview);
		...
		{
			ms_error("%s, selfview_item is NULL", __func__);
		}
	}
	else
	{
		ms_error("%s, main_window is NULL", __func__);
	}
	//ms_message("linphone_gtk_set_menu_items(%d), selfview=%d, video_enabled=%d", video_enabled, selfview, video_enabled);
}
...
static void linphone_gtk_configure_main_window(){
	...
	if (!config_loaded){
		title=linphone_gtk_get_ui_config("title","Linphone for MSCS (" LINPHONE_VERSION_DATE " )");
