...
void linphone_call_make_local_media_description(LinphoneCore *lc, LinphoneCall *call){
	...
	md->streams[0].maxptime=lc->net_conf.down_maxptime;
	l=make_codec_list(lc,lc->codecs_conf.audio_codecs,call->params.audio_bw,&md->streams[0].max_rate,-1);
	// audio codec 에 대해 8khz 와 16 khz 에 대한 telephone-event 를 각각 추가하여야 한다. RFC4733
	MSList *iter = NULL;
	PayloadType *tept = NULL, *maxpt = NULL;
	int		rate = 0;
	bool_t	bIsSetTe8k = FALSE, bIsSetTe16k = FALSE, bUseRfc2833 = TRUE;
	
	bUseRfc2833 = linphone_core_get_use_rfc2833_for_dtmf(lc);
	for(iter = l; iter!=NULL;iter=iter->next)
	{
		pt = (PayloadType*)iter->data;
		if (maxpt == NULL) maxpt = pt;
		else if (maxpt->normal_bitrate < pt->normal_bitrate)
		{
			maxpt = pt;
		}
		...
		{	// RFC2833 을 사용한다고 설정했을 때에만 telephone-event 를 추가한다.
			...
			if(tept != NULL)
			{
				...
				ms_message("Selected %s/%i, max_rate=%d,ptime=%d,maxptime=%d.", pt->mime_type, pt->clock_rate, rate, md->streams[0].ptime, md->streams[0].maxptime);
			}
			else
			{
				ms_message("No select telephone-event, max_rate=%d,ptime=%d.", rate, md->streams[0].ptime);
			}
			bIsSetTe8k = TRUE;
		}
	...
	if(maxpt)
	{
		linphone_core_update_allocated_audio_bandwidth_in_call(call, maxpt);
		md->streams[0].bandwidth = call->audio_bw;
		md->streams[0].rtcp_bw_rs = 0;
		md->streams[0].rtcp_bw_rr = (int)(md->streams[0].bandwidth*5);
	}
	...
	if (call->params.has_video){
		...
		md->streams[1].framerate=lc->video_conf.framerate;
	}
