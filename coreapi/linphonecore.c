...
static void net_config_read (LinphoneCore *lc)
{
	...
	tmp=lp_config_get_int(lc->config,"net","download_maxptime",0);
	linphone_core_set_download_maxptime(lc,tmp);
}
...
static void video_config_read(LinphoneCore *lc){
	...
	lc->video_conf.framerate=lp_config_get_int(lc->config,"video","framerate",0);
...
void linphone_core_set_download_maxptime(LinphoneCore *lc, int maxptime)
{
	lc->net_conf.down_maxptime = maxptime;
}
int linphone_core_get_download_maxptime(LinphoneCore *lc)
{
	return lc->net_conf.down_maxptime;
}
...
PayloadType	payload_type_telephone_event2={
...
static void linphone_core_init (LinphoneCore * lc, const LinphoneCoreVTable *vtable, LpConfig *config, void * userdata)
{
	...
	// change priority to compare telephone-event/16000 and telephone-event/8000
	linphone_core_assign_payload_type(lc, &payload_type_telephone_event2, -1, "0-11");		// telephone-event for AMR-WB
	linphone_core_assign_payload_type(lc,&payload_type_telephone_event, -1, "0-11");		// Org ; 101
...
#else	// LTE-PSVT 시험용 버전
	/**
	 *	User-agent: 지원서비스 - Network - OS - 단말모델명 - Client type - IMS 버전
	 *	지원서비스 = "VoLTE" | "PSVT" | "RCS" | "VoLTE, PSVT, RCS"
	 *	Network = "LTE" | "3G" | " WiFi"
	 *	OS = "iOS" | "And" | "WM"
	 *	단말모델명 = full name
	 *	Client type = "OEM" | "Client"
	 *	IMS 버전 = V1.0.0
	 */
	snprintf(ua_string, sizeof(ua_string)-1, "PSVT-WiFi-WM-%s-OEM-V%s", _ua_name, _ua_version);
#endif
...
/** Media Server 로 멘트 송출 요청을 전송한다.
 */
void linphone_core_send_aupkg(LinphoneCore *lc, char *pszCommand)
{
	LinphoneCall *call=linphone_core_get_current_call(lc);
	if (call==NULL){
		ms_warning("linphone_core_send_aupkg(): no active call");
		return;
	}
	sal_call_send_aupkg(call->op, pszCommand);
}
