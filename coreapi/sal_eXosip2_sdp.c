...
static int _sdp_message_get_a_int(sdp_message_t *sdp, int mline, char *szAttribute)
{
	int i,ret;
	sdp_attribute_t *attr;
	for (i=0;(attr=sdp_message_attribute_get(sdp,mline,i))!=NULL;i++){
		if (keywordcmp(szAttribute, attr->a_att_field)==0){
			int nb = sscanf(attr->a_att_value, "%i", &ret);
			/* the return value may depend on how %n is interpreted by the libc: see manpage*/
			if (nb == 1){
				return ret;
			}else ms_warning("sdp has a strange a=%s line (%s) ", szAttribute, attr->a_att_value);
		}
	}
	return 0;
}
...
{
	return _sdp_message_get_a_int(sdp, mline, "ptime");
}

static int _sdp_message_get_a_maxptime(sdp_message_t *sdp, int mline)
{
	return _sdp_message_get_a_int(sdp, mline, "maxptime");
}

static int _sdp_message_get_a_framerate(sdp_message_t *sdp, int mline)
{
	return _sdp_message_get_a_int(sdp, mline, "framerate");
}
...
static void add_line(sdp_message_t *msg, int lineno, const SalStreamDescription *desc){
	...
	if (desc->bandwidth>0)
		sdp_message_b_bandwidth_add (msg, lineno, osip_strdup ("AS"), int_2char(desc->bandwidth));
		
	if(desc->rtcp_bw_rs >= 0)
		sdp_message_b_bandwidth_add(msg,lineno,osip_strdup("RS"), int_2char(desc->rtcp_bw_rs));
		
	if(desc->rtcp_bw_rr >= 0)
		sdp_message_b_bandwidth_add(msg,lineno,osip_strdup("RR"), int_2char(desc->rtcp_bw_rr));
		
	if (desc->ptime>0)
		sdp_message_a_attribute_add(msg,lineno,osip_strdup("ptime"), int_2char(desc->ptime));
	
	if (desc->maxptime>0)
		sdp_message_a_attribute_add(msg,lineno,osip_strdup("maxptime"), int_2char(desc->maxptime));
...
int sdp_to_media_description(sdp_message_t *msg, SalMediaDescription *desc){
	...
	for (i=0; !sdp_message_endof_media (msg, i) && i<SAL_MEDIA_DESCRIPTION_MAX_STREAMS; i++)
	{
		...
		stream->maxptime=_sdp_message_get_a_maxptime(msg,i);
		stream->framerate = _sdp_message_get_a_framerate(msg, i);
