...
void register_set_contact_4service(osip_message_t *msg, osip_contact_t *ct);
void register_add_other_4service(osip_message_t *msg);
void invite_set_add_header_4service(osip_message_t *invite);
...
int sal_call(SalOp *h, const char *from, const char *to){
	...
	if (h->base.contact){
		...
		invite_set_add_header_4service(invite);
	}
...
/** SIP INFO 로 MediaServer 로 Audio Package 명령을 보내어서 멘트 송출을 지시한다
 * Command 에는 pa(an=BCN3009999990) 의 형식이어야 한다.
 */
int sal_call_send_aupkg(SalOp *h, char *pszCommand)
{
	osip_message_t *msg=NULL;
	char aupkg_body[1024] = "";
	char clen[10];
	...
	if (msg)
	{
		snprintf(aupkg_body, sizeof(aupkg_body), "%s\r\n", pszCommand);
		if(strstr(aupkg_body, "xml version") != NULL)
		{	// xml
			osip_message_set_content_type(msg, "application/ms-xml");
		}
		else
		{	// audio pkg
			osip_message_set_content_type(msg,"application/au");
		}
		osip_message_set_body(msg, aupkg_body, strlen(aupkg_body));
		snprintf(clen, sizeof(clen), "%lu", (unsigned long)strlen(aupkg_body));
		osip_message_set_content_length(msg, clen);
		eXosip_call_send_request(h->did, msg);
	}
	eXosip_unlock();
	...
	if (ev->request){
		if (MSG_IS_INFO(ev->request)){
			...
			if (ct && ct->subtype){
				...
				else if(strcmp(ct->subtype, "ms-xml") == 0)
				{	// INFO 메시지에 ms-xml 형태로 audio pkg가 오는 경우가 있음
					ms_message("Respone SIP INFO (ms-xml).");
					eXosip_lock();
					eXosip_call_build_answer(ev->tid,200,&ans);
					if (ans)
						eXosip_call_send_answer(ev->tid,200,ans);
				...
				else if(strcmp(ct->subtype, "AUPkg") == 0)
				{	// INFO 메시지의 audio pkg에 대한 응답을 수신하는 경우
					ms_message("Respone SIP INFO (AUPkg).");
					eXosip_lock();
					eXosip_call_build_answer(ev->tid,200,&ans);
					if (ans)
						eXosip_call_send_answer(ev->tid,200,ans);
				...
				else if(strcmp(ct->subtype, "au") == 0)
				{	// INFO 메시지의 audio pkg에 대한 응답을 수신하는 경우
					ms_message("Respone SIP INFO (au).");
					eXosip_lock();
					eXosip_call_build_answer(ev->tid,200,&ans);
					if (ans)
						eXosip_call_send_answer(ev->tid,200,ans);
...
static void register_set_contact(osip_message_t *msg, const char *contact){
	...
	register_set_contact_4service(msg, ct);
}
...
int sal_register(SalOp *h, const char *proxy, const char *from, int expires){
	...
	if (h->rid==-1){
		...
		if (msg){
			...
			register_add_other_4service(msg);
...
static char g_szAccessNetworkInfo[128] = "3GPP-E-UTRAN;utran-cell-id-3gpp=234151D0FCE11";
static char g_szUeIp[128] = "10.1.1.37";

void register_set_contact_4service(osip_message_t *msg, osip_contact_t *ct)
{
	ms_message("register_set_contact_4service(ct=0x%p)", ct);
	if(ct != NULL)
	{
		/** for PSVT/VoLTE
		 * reg-id=1;+sip,instance="<urn:uuid:00000000-0000-000000-00000000>" 
		 * methods="REGISTER,INVITE,ACK,BYE,CANCEL,REFER,OPTIONS,NOTIFY,SUBSCRIBE, MESSAGE,INFO,PUBLISH";
		 * audio;video;+g.3gpp.icsi-ref="urn%3Aurn-7%3A3gpp-service.ims.icsi.mmtel";
		 */
		osip_contact_param_add(ct, osip_strdup("reg-id"), osip_strdup("1"));
		osip_contact_param_add(ct, osip_strdup("+sip.instance"), osip_strdup("<urn:uuid:00000000-0000-000000-00000000>"));
		osip_contact_param_add(ct, osip_strdup("audio"), NULL);
		osip_contact_param_add(ct, osip_strdup("video"), NULL);
		
		/**
		 *	3.5.3.	ICSI(Feature Tag) 정보
		 ...
		 */
		osip_contact_param_add(ct, osip_strdup("+g.3gpp.icsi-ref"), osip_strdup("urn%3Aurn-7%3A3gpp-service.ims.icsi.mmtel"));
	}
}

void register_add_other_4service(osip_message_t *msg)
{
	ms_message("register_add_other_4service(msg=0x%p)", msg);
	if(msg != NULL)
	{
		/**
		 *	접속망
		 *	LTE		3GPP-E-UTRAN; utran-cell-id-3gpp=234151D0FCE11
		 *	WCDMA	3GPP-UTRAN; utran-cell-id-3gpp=3SK40FKW3454
		 *	WLAN	3GPP-WLAN; i-wlan-node-id=90SJW2F353192D12
		 ...
		 */
		osip_message_set_header(msg, osip_strdup("P-Access-Network-Info"), osip_strdup(g_szAccessNetworkInfo));
		osip_message_set_header(msg, osip_strdup("P-KT-UE-IP"), osip_strdup(g_szUeIp));
		...
		ms_message("register_add_other_4service.osip_authorization_init(0x%p)=%d", authorization, i);
		if (i == 0)
    	{
    		osip_authorization_set_auth_type(authorization, osip_strdup("Digest"));
			osip_authorization_set_algorithm(authorization, osip_strdup("MD5"));
			osip_authorization_set_realm(authorization, osip_strdup("ktann.com"));
			osip_authorization_set_uri(authorization, osip_strdup("sip:ktann.com"));
			osip_authorization_set_username(authorization, osip_strdup("11028557431@ktann.com"));
			
			i = osip_authorization_to_str (authorization, (char **)&szResult);
			ms_message("register_add_other_4service.osip_authorization_init=%d[%s]", i, szResult);
			
			msg->message_property = 2;
			osip_list_add (&msg->authorizations, authorization, -1);
		}
	}
}
...
void invite_set_add_header_4service(osip_message_t *invite)
{
	//osip_uri_t *pstReqUri = NULL;
	
	if(invite != NULL)
	{
		/** LTE-PSVT
		 *	PSVT 서비스를 지원하는 단말은 Initial Request 시에 요청하는 서비스에 대한 ICSI 정보를 Accept-Contact 헤더에 포함하여 전달한다
		 *	Accept-Contact: *;audio;video;+g.3gpp.icsi-ref="urn%3Aurn-7%3A3gpp-service.ims.icsi.mmtel";
		 */
		osip_message_set_header(invite, osip_strdup("Accept-Contact"), osip_strdup("*;audio;video;+g.3gpp.icsi-ref=\"urn%3Aurn-7%3A3gpp-service.ims.icsi.mmtel\";"));
		 ...
		 * Protocol (SIP) for the 3rd-Generation Partnership Project (3GPP)
		 */
		osip_message_set_header(invite, osip_strdup("P-Access-Network-Info"), osip_strdup(g_szAccessNetworkInfo));
		
		/** 회의 통화에 참여할 때에는 참여자 id를 Request-URI 에 추가하여야 한다.
		pstReqUri = osip_message_get_uri(invite);
		if(pstReqUri != NULL)
		{	// 회의 통화 참여자 id
			osip_uri_param_add(&pstReqUri->url_params, osip_strdup("leg_id"), osip_strdup("1"));
		}
		*/
	}
}
