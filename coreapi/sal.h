...
typedef struct SalStreamDescription{
	...
	int bandwidth;		// b=AS:768			(kbps)
	int ptime;			// a=ptime:20		(ms)	// RFC2327 ( RECOMMENDED duration of each packet in milliseconds )
	int maxptime;		// a=maxptime:240	(ms)	// RFC3555 ( maximum duration of each packet in milliseconds )
	int rtcp_bw_rs;		// b=RS:19200		(bps)	// RFC3556
	int	rtcp_bw_rr;		// b=RR:19200		(bps)	// RFC3556
	int framerate;		// a=framerate:15	(fps)	// RFC2327 ( the maximum video frame rate in frames/sec )
	SalStreamDir dir;
